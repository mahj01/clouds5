...
export class HistoriqueStatusUtilisateurService {
  constructor(
    @InjectRepository(HistoriqueStatusUtilisateur) private repo: Repository<HistoriqueStatusUtilisateur>,
    @InjectRepository(Utilisateur) private userRepo: Repository<Utilisateur>,
    @InjectRepository(StatutCompte) private statutRepo: Repository<StatutCompte>,
  ) {}
  ...
  async create(dto: CreateHistoriqueStatusUtilisateurDto) {
    ...
    const statut = await this.statutRepo.findOne({ where: { id: dto.statutCompteId } });
  ...
  async findOne(id: number) {
    const item = await this.repo.findOne({ where: { id }, relations: ['utilisateur', 'statut'] });
    if (!item) throw new NotFoundException('HistoriqueStatusUtilisateur not found');
    return item;
  }
