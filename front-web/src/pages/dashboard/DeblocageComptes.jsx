      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-white">
            <i className="fa fa-unlock-alt mr-2" aria-hidden="true" />
            Déblocage des comptes
          </h2>
          <p className="mt-2 text-sm text-slate-300">
            Comptes bloqués: <span className="font-semibold text-slate-100">{lockedCount}</span>
          </p>
        </div>
        ...
        <button
          ...
          onClick={load}
          disabled={loading}
          className="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/10 px-4 py-2 text-sm font-semibold text-white hover:bg-white/20 disabled:opacity-60"
        >
        ...
        <table className="w-full min-w-[760px] border-separate border-spacing-0">
          <thead>
            <tr className="text-left text-xs font-semibold uppercase tracking-wide text-slate-300">
              <th className="border-b border-white/10 px-3 py-3">ID</th>
              <th className="border-b border-white/10 px-3 py-3">Email</th>
              <th className="border-b border-white/10 px-3 py-3">Rôle</th>
              <th className="border-b border-white/10 px-3 py-3">Tentatives</th>
              <th className="border-b border-white/10 px-3 py-3">Bloqué le</th>
          ...
          <tbody>
            {items.length === 0 && !loading ? (
              <tr>
                <td className="px-3 py-5 text-sm text-slate-300" colSpan={6}>
                  Aucun compte bloqué.
                </td>
              </tr>
            ) : (
              items.map((u) => {
                ...
                return (
                  <tr key={u.id} className="text-sm text-slate-200">
                    <td className="border-b border-white/5 px-3 py-3">{u.id}</td>
                    <td className="border-b border-white/5 px-3 py-3">{u.email}</td>
                    <td className="border-b border-white/5 px-3 py-3">{u?.role?.nom || '—'}</td>
                    <td className="border-b border-white/5 px-3 py-3">{u.nbTentatives ?? '—'}</td>
                    <td className="border-b border-white/5 px-3 py-3">{formatDate(u.dateBlocage)}</td>
